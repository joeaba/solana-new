...
remote=https://github.com/axleiro/solana.git
...
if [[ -n $CI_BASE_BRANCH ]]; then
  ...
  echo "inside the CI_BASE_BRANCH"
elif [[ -n $CI_BRANCH ]]; then
  BRANCH="$CI_BRANCH"
  echo "inside the CI_BRANCH"
fi
echo branch="$BRANCH"
echo cibasebranch="$CI_BASE_BRANCH"
echo cibranch="$CI_BRANCH"
